...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            val isDarkTheme = isDarkThemeSaved()

            // Initialize theme on first load
            LaunchedEffect(Unit) {
                if (currentSettings == null && savedTheme != null) {
                    when (savedTheme) {
                        Theme.SolarizedLight -> settingsEventBus.updateTheme(savedTheme)
                        else -> settingsEventBus.updateDarkMode(isDarkTheme)
                    }
                }
            }

            TodoListTheme(
                darkTheme = currentSettings?.isDarkMode ?: (savedTheme != Theme.SolarizedLight && isDarkTheme),
                theme = currentSettings?.theme ?: if (savedTheme == Theme.SolarizedLight) savedTheme else null
            ) {
    ...
    private fun getSavedTheme(): Theme? {
        ...
        return prefHelper.getSelectedTheme()
    }
    ...
    private fun isDarkThemeSaved(): Boolean {
        ...
        return when (prefHelper.getSelectedTheme()) {
            ...
            Theme.Moon -> true
            Theme.SolarizedLight -> false // Not a dark theme
