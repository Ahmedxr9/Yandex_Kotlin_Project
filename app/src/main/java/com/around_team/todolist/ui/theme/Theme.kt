...
internal fun TodoListTheme(
    ...
    val colors = when (theme) {
        Theme.SolarizedLight -> {
            when (style) {
                JetTodoListStyle.Base -> solarizedLightPalette
            }
        }
        else -> {
            when (darkTheme) {
...
/**
 * Composable function that applies the Solarized Light theme.
 * Uses the Solarized Light color palette with Base3 (#fdf6e3) as background
 * and Blue (#268bd2) as primary color.
 *
 * @param content The composable content to display with the Solarized Light theme.
 */
@Composable
fun SolarizedLightTheme(content: @Composable () -> Unit) {
    TodoListTheme(
        style = JetTodoListStyle.Base,
        theme = Theme.SolarizedLight,
        darkTheme = false, // Not used when theme is specified
        content = content
    )
}
